# Getting Started

These guides will help you get started on creating your first mono app. We begin with guides that help you setup the toolchain and environment.

 1. [Install the frameworks](install.md)
 1. [The obligatory Hello World project](hello_world.md)
 1. [Your first app: Tic Tac Toe, pt. 1](tic-tac-toe-part-1.md)
 1. [Your first app: Tic Tac Toe, pt. 2](tic-tac-toe-part-2.md)
 1. [Your first app: Tic Tac Toe, pt. 3](tic-tac-toe-part-3.md)
 1. [Using Arduino IDE](arduino-hackers.md)
 1. [Misconceptions and Worst Practice](misconceptions.md)

### Monokiosk

 1. [Using Monokiosk](using_monokiosk.md)

